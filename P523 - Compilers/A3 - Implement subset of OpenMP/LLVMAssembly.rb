class LLVMAssembly
  ...
  define_rw_method :parsePidginC2 do |node|
	  puts "calling parsePidginC2"
	  puts node
	  alltd? node do |n|
  ...
  alltd? node do |n|
	...
	if match? :IfBlock[:condition,:body,:elsepart], n
		parsePidginC2 lookup(:body)
	end
