		...
		case SYS_exit:
			...
			DEBUG('e', "_exit() system call %d\n", p->pid);
                ...
                case SYS_write: 
                        ...
                        DEBUG('e', "write sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_write, p);
                        }
                        break;
                case SYS_read:
                        DEBUG('e', "read sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_read, p);
                        }
                        break;
                case SYS_ioctl:
                        DEBUG('e', "ioctl sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_ioctl, p);
                        }
                        break;
                case SYS_fstat:
                        DEBUG('e', "fstat sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_fstat, p);
                        }
                        break;
                case SYS_getpagesize:
                        DEBUG('e', "getpagesize sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_getpagesize, p);
                        }
                        break;
                case SYS_sbrk:
                        DEBUG('e', "sbrk sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_sbrk, p);
                        }
                        break;
                case SYS_execve:
                        DEBUG('e', "execve sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_execve, p);
                        }
                        break;
                case SYS_getpid:
                        DEBUG('e', "getpid sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_getpid, p);
                        }
                        break;
                case SYS_fork:
                        DEBUG('e', "fork sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_fork, p);
                        }
                        break;
                case SYS_getdtablesize:
                        DEBUG('e', "getdtablesize sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_getdtablesize, p);
                        }
                        break;
                case SYS_getppid:
                        DEBUG('e', "getppid sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_getppid, p);
                        }
                        break;
                case SYS_close:
                        DEBUG('e', "close sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_close, p);
                        }
                        break;
                case SYS_wait:
                        DEBUG('e', "wait sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_wait, p);
                        }
                        break;
                case SYS_dup2:
                        DEBUG('e', "dup sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_dup2, p);
                        }
                        break;
                case SYS_pipe:
                        DEBUG('e', "pipe sys call%d\n", p->pid);
                        if (p != NULL) {
                            mt_create(do_pipe, p);
                        }
                        break;
		default:
			DEBUG('e', "Unknown system call type %d\n", type);
			SYSHalt();
			break;
		}
		break;
